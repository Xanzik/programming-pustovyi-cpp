...
Watch& List::operator[] (const int index) {
    return watchlist[index];
}
ofstream& operator <<(ofstream &out, const List &list) {
    out << list.watchlist->getName() << " " << list.watchlist->getWater() << " " << list.watchlist->getPrice() << "$" << " " << " " << list.watchlist->getType() << std::endl;
    return out;
}
size_t List::getSize() {
    return size;
...
ifstream& operator >>(ifstream &in, List &list) {
    ...
    string type;
    while(!in.eof()){
    in >> name >> water >> price >> firma >> country >> type;
    Watch *watches = new Watch(name, water, price, firma, country, type);
    list.addWatch(watches, 0);
    }
    return in;
}